...
if [ -f "ml.zip" ]; then
    echo "Unzipping ML into card..."
    mkdir /tmp/mlinstall_output
    mount $dev /tmp/mlinstall_output
    unzip ml.zip -d /tmp/mlinstall_output
    umount /tmp/mlinstall_output
fi
...
def install():
    if os.name == "posix":
        f = open("bootable.sh", "w")
        f.write(linux_script)
        f.close()
        os.system("chmod +x bootable.sh")
        r = os.system("./bootable.sh")
        os.remove("bootable.sh")
        return r
    else:
