...
static inline uint16_t htod16p(PTPParams *params, uint16_t var)
{
	return ((params->byteorder == PTP_DL_LE) ? htole16(var) : htobe16(var));
}

static inline uint32_t htod32p(PTPParams *params, uint32_t var)
{
	return ((params->byteorder == PTP_DL_LE) ? htole32(var) : htobe32(var));
}

static inline void htod16ap(PTPParams *params, unsigned char *a, uint16_t val)
{
...
static inline void htod32ap(PTPParams *params, unsigned char *a, uint32_t val)
{
	if (params->byteorder == PTP_DL_LE)
		htole32a(a, val);
	else
...
static inline uint16_t dtoh16p(PTPParams *params, uint16_t var)
{
	return ((params->byteorder == PTP_DL_LE) ? le16toh(var) : be16toh(var));
}

static inline uint32_t dtoh32p(PTPParams *params, uint32_t var)
{
	return ((params->byteorder == PTP_DL_LE) ? le32toh(var) : be32toh(var));
}

static inline uint16_t dtoh16ap(PTPParams *params, unsigned char *a)
{
	return ((params->byteorder == PTP_DL_LE) ? le16atoh(a) : be16atoh(a));
}

static inline uint32_t dtoh32ap(PTPParams *params, unsigned char *a)
{
...
static inline char *ptp_unpack_string(PTPParams *params, char *data, uint16_t offset, uint8_t *len)
{
	int i;
	char *string = NULL;
...
static inline void ptp_pack_string(PTPParams *params, char *string, char *data, uint16_t offset,
				   uint8_t *len)
{
	int i;
...
static inline uint32_t ptp_unpack_uint32_t_array(PTPParams *params, char *data, uint16_t offset,
						 uint32_t **array)
{
	uint32_t n, i = 0;
...
static inline uint32_t ptp_unpack_uint16_t_array(PTPParams *params, char *data, uint16_t offset,
						 uint16_t **array)
{
	uint32_t n, i = 0;
...
static inline void ptp_unpack_DI(PTPParams *params, char *data, PTPDeviceInfo *di)
{
	uint8_t len;
	...
	di->VendorExtensionDesc = ptp_unpack_string(params, data, PTP_di_VendorExtensionDesc, &len);
	totallen = len * 2 + 1;
	di->FunctionalMode = dtoh16a(&data[PTP_di_FunctionalMode + totallen]);
	di->OperationsSupported_len = ptp_unpack_uint16_t_array(
		params, data, PTP_di_OperationsSupported + totallen, &di->OperationsSupported);
	totallen = totallen + di->OperationsSupported_len * sizeof(uint16_t) + sizeof(uint32_t);
	di->EventsSupported_len = ptp_unpack_uint16_t_array(
		params, data, PTP_di_OperationsSupported + totallen, &di->EventsSupported);
	totallen = totallen + di->EventsSupported_len * sizeof(uint16_t) + sizeof(uint32_t);
	di->DevicePropertiesSupported_len =
		ptp_unpack_uint16_t_array(params, data, PTP_di_OperationsSupported + totallen,
					  &di->DevicePropertiesSupported);
	totallen =
		totallen + di->DevicePropertiesSupported_len * sizeof(uint16_t) + sizeof(uint32_t);
	di->CaptureFormats_len = ptp_unpack_uint16_t_array(
		params, data, PTP_di_OperationsSupported + totallen, &di->CaptureFormats);
	totallen = totallen + di->CaptureFormats_len * sizeof(uint16_t) + sizeof(uint32_t);
	di->ImageFormats_len = ptp_unpack_uint16_t_array(
		params, data, PTP_di_OperationsSupported + totallen, &di->ImageFormats);
	totallen = totallen + di->ImageFormats_len * sizeof(uint16_t) + sizeof(uint32_t);
	di->Manufacturer =
		ptp_unpack_string(params, data, PTP_di_OperationsSupported + totallen, &len);
	totallen += len * 2 + 1;
	di->Model = ptp_unpack_string(params, data, PTP_di_OperationsSupported + totallen, &len);
	totallen += len * 2 + 1;
	di->DeviceVersion =
		ptp_unpack_string(params, data, PTP_di_OperationsSupported + totallen, &len);
	totallen += len * 2 + 1;
...
static inline void ptp_unpack_OH(PTPParams *params, char *data, PTPObjectHandles *oh)
{
...
static inline void ptp_unpack_SIDs(PTPParams *params, char *data, PTPStorageIDs *sids)
{
	sids->n = ptp_unpack_uint32_t_array(params, data, PTP_sids, &sids->Storage);
}
...
static inline void ptp_unpack_SI(PTPParams *params, char *data, PTPStorageInfo *si)
{
	...
	si->StorageDescription =
		ptp_unpack_string(params, data, PTP_si_StorageDescription, &storagedescriptionlen);
	si->VolumeLabel =
...
static inline uint32_t ptp_pack_OI(PTPParams *params, PTPObjectInfo *oi, char **oidataptr)
{
	char *oidata;
	uint8_t filenamelen;
...
static inline void ptp_unpack_OI(PTPParams *params, char *data, PTPObjectInfo *oi)
{
	uint8_t filenamelen;
	...
	capture_date = ptp_unpack_string(params, data, PTP_oi_filenamelen + filenamelen * 2 + 1,
					 &capturedatelen);
	...
	capture_date =
		ptp_unpack_string(params, data,
				  PTP_oi_filenamelen + filenamelen * 2 + capturedatelen * 2 + 2,
				  &capturedatelen);
	if (capturedatelen > 15) {
		strncpy(tmp, capture_date, 4);
		tmp[4] = 0;
...
#define CTVAL(type, func, target)                                                                  \
	{                                                                                          \
		...
		**(type **)target = func(data);                                                    \
	}

static inline void ptp_unpack_DPV(PTPParams *params, char *data, void **value, uint16_t datatype)
{
	switch (datatype) {
	case PTP_DTC_INT8:
...
#define CTVA(type, func, target)                                                                   \
	{                                                                                          \
		target = malloc(sizeof(type));                                                     \
		*(type *)target = func(&data[PTP_dpd_FactoryDefaultValue + totallen]);             \
...
#define MCTVA(type, func, target, n)                                                               \
	{                                                                                          \
		...
		for (i = 0; i < n; i++) {                                                          \
			...
			*(type *)target[i] = func(&data[PTP_dpd_FactoryDefaultValue + totallen]);  \
			totallen += sizeof(type);                                                  \
		}                                                                                  \
	}

static inline void ptp_unpack_DPD(PTPParams *params, char *data, PTPDevicePropDesc *dpd)
{
	uint8_t len;
	int totallen = 0;
	...
	case PTP_DTC_STR:
		dpd->FactoryDefaultValue =
			(void *)ptp_unpack_string(params, data, PTP_dpd_FactoryDefaultValue, &len);
		totallen = len * 2 + 1;
		dpd->CurrentValue = (void *)ptp_unpack_string(
			params, data, PTP_dpd_FactoryDefaultValue + totallen, &len);
		totallen += len * 2 + 1;
		break;
	}
...
#define N dpd->FORM.Enum.NumberOfValues
		...
		case PTP_DTC_STR: {
			int i;
			for (i = 0; i < N; i++) {
				*(char **)&dpd->FORM.Enum.SupportedValue[i] = ptp_unpack_string(
					params, data, PTP_dpd_FactoryDefaultValue + totallen, &len);
				totallen += len * 2 + 1;
			}
		} break;
...
static inline uint32_t ptp_pack_DPV(PTPParams *params, void *value, char **dpvptr,
				    uint16_t datatype)
{
	char *dpv = NULL;
	uint32_t size = 0;
...
static inline void ptp_unpack_EC(PTPParams *params, char *data, PTPUSBEventContainer *ec)
{
	if (data == NULL)
		return;
...
static inline void ptp_unpack_Canon_FE(PTPParams *params, char *data, PTPCANONFolderEntry *fe)
{
	int i;
	if (data == NULL)
		return;
...
static inline void ptp_nikon_unpack_EC(PTPParams *params, char *evdata,
				       PTPUSBEventContainer **event, uint16_t *evnum)
{
	...
	*event = events;
	while (i < *evnum) {
		events->code = dtoh16a(
			&evdata[PTP_NIKON_ec_Code + ((sizeof(uint16_t) + sizeof(uint32_t)) * i)]);
		events->param1 = dtoh32a(
			&evdata[PTP_NIKON_ec_Param1 + ((sizeof(uint16_t) + sizeof(uint32_t)) * i)]);
		events++;
		i++;
	}
}
